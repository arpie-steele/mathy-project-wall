...
if ( 0 ) {
    ...
    add_logical_variable('sigma', "\N{MATHEMATICAL ITALIC SMALL SIGMA}", 'si');
    add_logical_variable('rho', "\N{MATHEMATICAL ITALIC SMALL RHO}", 'rh');
    add_logical_variable('mu', "\N{MATHEMATICAL ITALIC SMALL MU}", 'mu');
    add_logical_variable('lamda', "\N{MATHEMATICAL ITALIC SMALL LAMDA}", 'la');
    add_logical_variable('kappa', "\N{MATHEMATICAL ITALIC SMALL KAPPA}", 'ka');
}

initialize_representations();
# print Dumper([$all_vars_true, @logical_vars]);

add_logical_op(2, 'iff', "\N{LEFT RIGHT ARROW}", '<->', sub { return ( ~ ($_[0] ^ $_[1]) & $all_vars_true ); });
add_logical_op(2, 'implies', "\N{RIGHTWARDS ARROW}", '->', sub { return ( ( (~ $_[0]) & $all_vars_true ) | $_[1] ); });
add_logical_op(1, 'not', "\N{NOT SIGN}", '-.', sub { return ((~ $_[0]) & $all_vars_true); } );
if (0) {
    add_logical_op(2, 'or', "\N{LOGICAL OR}", '\\/', sub { return $_[0] | $_[1]; });
    add_logical_op(3, 'or3', "\N{LOGICAL OR}", '\\/', sub { return $_[0] | $_[1] | $_[2]; });
    add_logical_op(2, 'and', "\N{LOGICAL AND}", '/\\', sub { return $_[0] & $_[1]; });
    add_logical_op(3, 'and3', "\N{LOGICAL AND}", '/\\', sub { return ($_[0] & $_[1] & $_[2]); });
    add_logical_op(4, 'and4', "\N{LOGICAL AND}", '/\\', sub { return $_[0] & $_[1] & $_[2] & $_[3]; });
    add_logical_op(2, 'nand', "\N{NAND}", '-/\\', sub { return ( ~ ($_[0] & $_[1]) & $all_vars_true ); });
    add_logical_op(3, 'nand3', "\N{NAND}", '-/\\', sub { return ( ~ ($_[0] & $_[1] & $_[2]) & $all_vars_true ); });
    add_logical_op(2, 'xor', "\N{XOR}", '\\/_', sub { return ($_[0] ^ $_[1]) & $all_vars_true; });
    add_logical_func(3, 'hadd', sub { return ( ($_[0] ^ $_[1]) ^ $_[2] ) & $all_vars_true; });
    add_logical_func(3, 'cadd', sub { return ( ($_[0] & $_[1]) | ( $_[2] & ( $_[0] | $_[1]) ) ); });
    add_logical_func(3, 'if-', sub { return ( $_[0] & $_[1] ) | ( (~ $_[0]) & $_[2] ); });
    add_logical_op(0, 'TRUE', "\N{DOWN TACK}", 'T.', sub { return $all_vars_true; } );
    add_logical_op(0, 'FALSE', "\N{UP TACK}", 'F.', sub { return ((~ $all_vars_true) & $all_vars_true ); } );
}
...

{
    ...
    foreach my $i ( 0..($n-1) ) {
	print "$logical_vars[$i]{'nam'} is $logical_vars[$i]{'asc'} or $logical_vars[$i]{'utf'} in UNICODE.\n";
	# print Dumper($logical_vars[$i]);
    }
    my $m = scalar @logical_ops;
    foreach my $i ( 1..$m ) {
	print "$logical_ops[-$i]{'nam'} is $logical_ops[-$i]{'asc'} or $logical_ops[-$i]{'utf'} in UNICODE.\n";
...
while (scalar @wip ) {
	...
	foreach my $row_index ( 0..$db_last ) {
		...
		print "WEEDING slots: @{[scalar @slots]}, FROM: $row->[4] : $row->[1]\n";
		$proofs = 1;
	    }
	    last;
	}
...
foreach my $row ( sort  { $a->[5] <=> $b->[5] || $a->[4] cmp $b->[4] } @database ) {
...
sub load_data {
	...
	if ( exists $lookup{$name} ) { croak "COLLISION at name $name between $i and $lookup{$name}\n"; }
	$lookup{$name} = $i;
    }
    my $m = scalar @logical_ops;
    foreach my $i ( 1..$m ) {
	...
	foreach my $word ( @words ) {
	    if ( ! exists $lookup{$word} ) { carp "in $label: Unknown symbol: $word\n"; $error = 1; last; }
	...
	if ( ! $error ) {
	    ...
	    if (
	    $is_numeric_representation && $representaion_value == $all_vars_true 
	    || !$is_numeric_representation && $representaion_value eq $all_vars_true  ) {
...
sub infix {
	...
	if ( $node < 0 ) {
	    ...
	    if ( $is_func ) {
		unshift @infix_stack, $display . ' ( ' . ( join " , ", @params ) . ' )';
	    } elsif ( $arity == 0 ) {
		unshift @infix_stack, $display;
	    } elsif ( $arity == 1 ) {
		unshift @infix_stack, join q( ), $display, @params;
	    } else {
		unshift @infix_stack, '( ' . ( join " $display ", @params ) . ' )';
	    }
...
sub infix_pat {
	...
	if ( $node < 0 ) {
	    ...
	    my @params;
	    if ( $arity ) {
		push @params, map { ( ',' , @{ $_ } ); } splice @infix_stack, 0, $arity;
	    }
...
sub initialize_representations {
	...
	if ( $index > 2 ) {
	    ...
	    $representation = ( "\x00" x $half_width ) . ("\xff" x $half_width);
...
sub prove_and_add {
	...
	if ( $proofs && 1 < scalar @{$proofs} ) {
		...
		print "PROVE FROM: ", $database[$ri][4], ": ", $database[$ri][1], "\n";
	    }
	}
	...
	if ( ! defined $proofs ) {
	    ...
	    add_to_database($tree, $label, $pattern, $infix, $polish, $proofs);
	}
    }
    return;
}
...
sub add_to_database {
	...
	push @database, [ qr/\A$pattern\z/xms, $infix, $polish, $proofs, $label, 0, [ @{$tree} ], $max_var];
    }
    ...
    if ( 2 + (scalar @{$tree} )  <= $length_limit && ( exists $lookup{'iff'} || exists $lookup{'and'} ) ) {
	my $count = 0;
	foreach my $ri ( 0..( (scalar @database) - 1 ) ) {
	    my $row = $database[$ri];
	    if ( $max_var + 1 + $row->[7] < $n && 1 + (scalar @{$tree} ) + (scalar @{$row->[6]}) <= $length_limit ) {
		my @extra = map { $_ >= 0 ? $_ + 1 + $max_var : $_ } @{$row->[6]};
		my @extra_proofs = ( @$new_proofs, $ri );
		if ( $row->[3] ) { push @extra_proofs, @{$row->[3]}; }

		if ( exists $lookup{'iff'} ) {
		    prove_and_add( [ $lookup{'iff'}, @{$tree}, @extra], $label . 'X' . $count, undef, undef, undef, \@extra_proofs);
		    $count ++;
		}
		if ( exists $lookup{'and'} ) {
		    prove_and_add( [ $lookup{'and'}, @{$tree}, @extra], $label . 'X' . $count, undef, undef, undef, \@extra_proofs);
		    $count ++;
		} 
	    }
	}
	...
	my ( $index, $want ) = (1, 1);
	while ( $want ) {
	    if ( $tree->[$index] < 0 ) { $want += $logical_ops[$tree->[$index]]->{'nsl'} ; }
	    $want --;
	    $index ++;
	}
	my @new_tree = @{$tree};
	...
	my %variable_map; my $next_variable = 0;
	foreach my $node ( @new_tree ) {
	    if ( $node >= 0 ) {
		if ( ! exists $variable_map{$node} ) { $variable_map{$node} = $next_variable; $next_variable ++; }
	    }
	}
	@new_tree = map { $_ < 0 ? $_ : $variable_map{$_}; } @new_tree;
	prove_and_add( \@new_tree, $label . 'C', undef, undef, undef, $new_proofs );
    }
    # 
    if ( (scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'not'} && $tree->[0] == $lookup{'not'}
    && ( 
	...
	@new_tree = map { $_ < 0 ? $_ : $variable_map{$_}; } @new_tree;
	prove_and_add( \@new_tree, $label . 'C3', undef, undef, undef, $new_proofs );
    }
    # biimpi ⊢ (𝜑 ↔ 𝜓)    ⇒   ⊢ (𝜑 → 𝜓)
    if ( (scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} && exists $lookup{'iff'}
    && $tree->[0] == $lookup{'iff'}
    ) {
	prove_and_add( [ $lookup{'implies'}, ( map { $tree->[$_]; } ( 1..((scalar @{$tree}-1)) ) ) ], $label . 'B', undef, undef, undef, $new_proofs );
    }
    # biimpd ⊢ (𝜑 → (𝜓 ↔ 𝜒))    ⇒   ⊢ (𝜑 → (𝜓 → 𝜒))
    if ( (scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} && exists $lookup{'iff'}
	...
	if ( $tree->[$index] == $lookup{'iff'} ) {
	    prove_and_add( [ ( map { ( $_ == $index ) ? $lookup{'implies'} : $tree->[$_]; } ( 0..((scalar @{$tree}-1)) ) ) ], $label . 'B2', undef, undef, undef, $new_proofs );
	}
    }
    # pm2.21i ⊢ ¬ 𝜑    ⇒   ⊢ (𝜑 → 𝜓)
    if ( (1 + scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} && exists $lookup{'not'}
    && $tree->[0] == $lookup{'not'} && $tree->[1] != $lookup{'not'}
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'implies'}, ( map { $tree->[$_]; } ( 1..((scalar @{$tree}-1)) ) ), $max_var + 1 ], $label . 'D3', undef, undef, undef, $new_proofs );
    }
    if ( (1 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'iff'} && exists $lookup{'FALSE'} && exists $lookup{'not'}
    && $tree->[0] == $lookup{'not'}
    ) {
	prove_and_add( [ $lookup{'iff'}, $lookup{'FALSE'}, ( map { $tree->[$_]; } ( 1..((scalar @{$tree}-1)) ) )], $label . 'B4', undef, undef, undef, $new_proofs );
    }
    # a1i ⊢ 𝜑    ⇒   ⊢ (𝜓 → 𝜑)
    if ( 
    exists $lookup{'implies'} 
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'implies'}, 0, map { ( $_ >= 0 ) ? 1 + $_ : $_ ; } @{$tree}], $label . 'D', undef, undef, undef, $new_proofs );
    }
    #  ⊢ (𝜑 → 𝜓)    ⇒   ⊢ (¬ 𝜓 → ¬ 𝜑)
    if ( (2 + scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} && $tree->[0] == $lookup{'implies'} && exists $lookup{'not'}
    ) {
	my ( $index, $want ) = (1, 1);
	while ( $want ) {
	    if ( $tree->[$index] < 0 ) { $want += $logical_ops[$tree->[$index]]->{'nsl'} ; }
	...
	@new_tree = map { $_ < 0 ? $_ : $variable_map{$_}; } @new_tree;
	prove_and_add( \@new_tree, $label . 'C2', undef, undef, undef, $new_proofs );
    }
    # notnoti ⊢ 𝜑    ⇒   ⊢ ¬ ¬ 𝜑
    if ( (2 + scalar @{$tree} ) <=  $length_limit && exists $lookup{'not'} ) {
	prove_and_add( [ $lookup{'not'}, $lookup{'not'}, @{$tree} ], $label . 'N2', undef, undef, undef, $new_proofs );
    }
    #
    if ( (2 + scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'or'}
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'or'}, @{$tree}, $max_var + 1 ], $label . 'O2', undef, undef, undef, $new_proofs );
    }
    #
    if ( (2 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'and'} && exists $lookup{'TRUE'}
    ) {
	# prove_and_add( [ $lookup{'and'}, $lookup{'TRUE'}, @{$tree}], $label . 'A', undef, undef, undef, $new_proofs );
    }
    if ( (2 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'iff'} && exists $lookup{'TRUE'}
    ) {
    ...
    && exists $lookup{'or'} 
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'or'}, 0, map { ( $_ >= 0 ) ? 1 + $_ : $_ ; } @{$tree}], $label . 'O', undef, undef, undef, $new_proofs );
    }
    #
    if ( (3 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'and'} && exists $lookup{'FALSE'} && exists $lookup{'not'}
    ) {
	prove_and_add( [ $lookup{'and'}, $lookup{'not'}, $lookup{'FALSE'}, @{$tree}], $label . 'A2', undef, undef, undef, $new_proofs );
    }
    # pm2.24i ⊢ 𝜑    ⇒   ⊢ (¬ 𝜑 → 𝜓)
    if ( (3 + scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} && exists $lookup{'not'}
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'implies'}, $lookup{'not'}, @{$tree}, $max_var + 1 ], $label . 'D2', undef, undef, undef, $new_proofs );
    }
    #
    if ( (4 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'and'} && exists $lookup{'TRUE'} && exists $lookup{'not'}
    ) {
	prove_and_add( [ $lookup{'and'}, $lookup{'not'}, $lookup{'not'}, $lookup{'TRUE'}, @{$tree}], $label . 'A3', undef, undef, undef, $new_proofs );
    }
    #  ⊢ 𝜑    ⇒   ⊢ ( ( 𝜑 → 𝜓 ) → 𝜓 )
    if ( (4 + scalar @{$tree} ) <=  $length_limit
    && exists $lookup{'implies'} 
    && $max_var + 1 < $n ) {
	prove_and_add( [ $lookup{'implies'}, $lookup{'implies'}, @{$tree}, $max_var + 1, $max_var + 1], $label . 'I2', undef, undef, undef, $new_proofs );
    }
    #
    if ( (5 + scalar @{$tree} ) <= $length_limit
    && exists $lookup{'and'} && exists $lookup{'FALSE'} && exists $lookup{'not'}
    ) {
	prove_and_add( [ $lookup{'and'}, $lookup{'not'}, $lookup{'not'}, $lookup{'not'}, $lookup{'FALSE'}, @{$tree}], $label . 'A4', undef, undef, undef, $new_proofs );
    }
